...
import { AzureDevOpsAuthenticationError, AzureDevOpsError } from '../errors';
...
jest.mock('../auth/client-factory');

const MockSharedClient = SharedClient as jest.MockedClass<typeof SharedClient>;
...
describe('AzureDevOpsClient', () => {
  ...
  let mockSharedClientInstance: any;
  ...
  let mockGetWebApiClient: jest.Mock;
  ...
  beforeEach(() => {
    ...
    mockGetWebApiClient = jest.fn().mockResolvedValue({});
    ...
    mockSharedClientInstance = {
      getWebApiClient:
        mockGetWebApiClient.mockResolvedValue(mockWebApiInstance),
      getCoreApi: mockGetCoreApi,
      getGitApi: mockGetGitApi,
      getWorkItemTrackingApi: mockGetWorkItemTrackingApi,
      getBuildApi: mockGetBuildApi,
    ...
    MockSharedClient.mockImplementation(() => mockSharedClientInstance);

    client = new AzureDevOpsClient(config);
  });

  describe('getClient', () => {
    it('should create SharedClient only once', async () => {
      await client.getCoreApi();
      await client.getCoreApi();

      expect(MockSharedClient).toHaveBeenCalledTimes(1);
      expect(MockSharedClient).toHaveBeenCalledWith({
    ...
    it('should handle authentication errors when creating client', async () => {
      mockGetWebApiClient.mockRejectedValueOnce(
    ...
    it('should handle generic errors when creating client', async () => {
      mockGetWebApiClient.mockRejectedValueOnce(new Error('Network error'));
    ...
    it('should handle non-Error instances when creating client', async () => {
      mockGetWebApiClient.mockRejectedValueOnce('String error');
  ...
  describe('isAuthenticated', () => {
    it('should return true if client is accessible', async () => {
      const isAuth = await client.isAuthenticated();
      expect(isAuth).toBe(true);
      expect(MockSharedClient).toHaveBeenCalledTimes(1);
    });

    it('should return false if client is not accessible', async () => {
      mockGetWebApiClient.mockRejectedValueOnce(new Error('API Error'));
      const isAuth = await client.isAuthenticated();
