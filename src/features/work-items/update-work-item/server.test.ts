...
// Mock the updateWorkItem feature
jest.mock('./feature', () => {
...
const { updateWorkItem } = require('./feature');
...
describe('updateWorkItem', () => {
  ...
  it('should update a work item with basic fields', async () => {
    ...
    updateWorkItem.mockImplementationOnce(
      async (_: any, workItemId: number, fields: any) => {
        // Call the mocked API
        ...
        return await mockWorkItemTrackingApi.updateWorkItem(
          document,
          workItemId,
        );
      },
    ...
    const document: JsonPatchDocument =
      mockWorkItemTrackingApi.updateWorkItem.mock.calls[0][0];
    expect(document).toBeInstanceOf(Array);

    // Verify title operation
    const titleOperation = document.find(
      (op: JsonPatchOperation) => op.path === '/fields/System.Title',
    );
    ...
    const descriptionOperation = document.find(
      (op: JsonPatchOperation) => op.path === '/fields/System.Description',
    );
  ...
  it('should update a work item with all fields', async () => {
    ...
    const assignedToOperation = document.find(
      (op: JsonPatchOperation) => op.path === '/fields/System.AssignedTo',
    );
    expect(assignedToOperation?.value).toBe('user@example.com');

    const areaPathOperation = document.find(
      (op: JsonPatchOperation) => op.path === '/fields/System.AreaPath',
    );
    expect(areaPathOperation?.value).toBe('testproject\\Team A');

    const iterationPathOperation = document.find(
      (op: JsonPatchOperation) => op.path === '/fields/System.IterationPath',
    );
    expect(iterationPathOperation?.value).toBe('testproject\\Sprint 1');

    const priorityOperation = document.find(
      (op: JsonPatchOperation) =>
        op.path === '/fields/Microsoft.VSTS.Common.Priority',
    );
    expect(priorityOperation?.value).toBe(1);

    const stateOperation = document.find(
      (op: JsonPatchOperation) => op.path === '/fields/System.State',
    );
    expect(stateOperation?.value).toBe('Active');

    const customFieldOperation = document.find(
      (op: JsonPatchOperation) => op.path === '/fields/Custom.Field',
    );
  ...
  it('should throw AzureDevOpsResourceNotFoundError when work item is not found', async () => {
    ...
    mockWorkItemTrackingApi.updateWorkItem.mockRejectedValueOnce(
    ...
    await expect(
      updateWorkItem(mockConnection, 999, {
        title: 'Updated Work Item',
      }),
  ...
  it('should throw an error when API call fails', async () => {
    ...
    // Call updateWorkItem and expect it to throw
    await expect(
      updateWorkItem(mockConnection, 123, {
        title: 'Updated Work Item',
      }),
